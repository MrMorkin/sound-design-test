...
namespace Invector.vCharacterController.AI
{
    ...
    public class vControlAIShooter : vControlAICombat, vIControlAIShooter
    {
        ...
        public IK.vIKSolver leftIK { get; set; }
        public IK.vIKSolver rightIK { get; set; }
        ...
        public void CheckCanShot()
        {
            ...
            if (_waitingToShot < Time.time && !inTurn && (isStrafing || debugShoots|| input.magnitude<0.1f)|| forceCanShot)
            {
            ...
            if (currentTarget.transform||(debugShoots&&lockAimDebug))
            {
                ...
                if (_canShot)
                {
                    ...
                    else
                    {
                        ...
                        if (secundaryShots == 0 || !inShotDelay)
                            secundaryShots++;
                    }
            ...
            if (shooterManager && isAiming && _canAiming)
            {
                ...
                if (weapon)
                {
                    ...
                    if ( debugAim)
                    {
                        ...
                        Debug.DrawRay(weapon.aimReference.position, (aimTarget - weapon.aimReference.position), Color.yellow);
                        Debug.DrawRay(weapon.muzzle.position, weapon.muzzle.forward*100f, Color.blue);
                    }
                    return;
                }
            }
        ...
        protected virtual void RotateAimArm(bool isUsingLeftHand = false)
        {
            ...
            if (CurrentActiveWeapon && armAlignmentWeight > 0.1f && CurrentActiveWeapon.alignRightUpperArmToAim)
            {
                ...
                if (!shooterManager.isShooting &&(!float.IsNaN(rot.x) && !float.IsNaN(rot.y) && !float.IsNaN(rot.z)))
                    upperArmRotationAlignment =inTurn? upperArmRotation: rot;
        ...
        protected virtual void RotateAimHand(bool isUsingLeftHand = false)
        {
            ...
            if (CurrentActiveWeapon && armAlignmentWeight > 0.1f && _canAiming && CurrentActiveWeapon.alignRightHandToAim)
            {
                ...
                if (!shooterManager.isShooting && (!float.IsNaN(rot.x) && !float.IsNaN(rot.y) && !float.IsNaN(rot.z)))
                    handRotationAlignment =inTurn? handRotation: rot;
                var angle = Vector3.Angle(AimPositionClamped() - aimAngleReference.transform.position, aimAngleReference.transform.forward);
                if ((!(angle > shooterManager.maxHandAngle || angle < -shooterManager.maxHandAngle)) )
        ...
        public virtual void Shot()
        {
            ...
            if (_canShot && !IsReloading && !waitingReload && inShotDelay && _canAiming && IsInShotAngle && isAiming &&!inTurn)
            {
            ...
            if (!_canShot && !IsReloading && !waitingReload && doReloadWhileWaiting && shooterManager.currentWeapon.ammoCount < shooterManager.currentWeapon.clipSize)
            {
