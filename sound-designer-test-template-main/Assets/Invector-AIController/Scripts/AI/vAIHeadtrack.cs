    ...
    public class vAIHeadtrack : vMonoBehaviour
    {
        ...
        protected virtual void LookAtIK(Vector3 point, float strafeHeadWeight, float spineWeight)
        {
            if (isLookingForSomething || !autoCancelUpdateIK)
            {
                ...
                xAngle = NormalizeAngle(xAngle) + Quaternion.Euler(offsetHead).eulerAngles.x;
                yAngle = NormalizeAngle(yAngle) + Quaternion.Euler(offsetHead).eulerAngles.y;
        ...
        {
            get { return followCamera&& mainCamera ? (mainCamera.transform.position + (mainCamera.transform.forward * 100)) : headPoint + (transform.forward * 100); }
        }
